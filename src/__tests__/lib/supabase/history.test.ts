...
describe("aggregateMonthEntries", () => {
  it("groups journal and manifest entries by date", () => {
    ...
    expect(result["2026-06-15"]).toEqual({ date: "2026-06-15", journalCount: 2, manifestCount: 1, lifeLogCount: 0 });
    expect(result["2026-06-16"]).toEqual({ date: "2026-06-16", journalCount: 1, manifestCount: 0, lifeLogCount: 0 });
    expect(result["2026-06-17"]).toEqual({ date: "2026-06-17", journalCount: 0, manifestCount: 1, lifeLogCount: 0 });
  });
